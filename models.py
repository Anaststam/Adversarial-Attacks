...
class SmoothClassifier(nn.Module):
    ...
    def certify(self, inputs: torch.Tensor, n0: int, num_samples: int, alpha: float, batch_size: int) -> Tuple[
        int, float]:
        """
        ...
        ##########################################################
        # YOUR CODE HERE
        class_count = self._sample_noise_predictions(inputs, n0, batch_size)
        top_class = class_count.argmax()
        class_count2 = self._sample_noise_predictions(inputs, num_samples, batch_size)
        top_class_count = class_count2[top_class]
        p_A_lower_bound = lower_confidence_bound(top_class_count.numpy(), num_samples, alpha)
        ##########################################################
    ...
    def predict(self, inputs: torch.tensor, num_samples: int, alpha: float, batch_size: int) -> int:
        ...
        ##########################################################
        # YOUR CODE HERE
        label = class_counts.argmax()
        count1 = class_counts[label]
        class_counts[label] = 0
        count2 = class_counts.max()
        if binom_test(count1, count1.numpy() + count2.numpy(), p=0.5) <= alpha:
            return label.numpy()
        else:
            return -1
        ##########################################################
    ...
    def _sample_noise_predictions(self, inputs: torch.tensor, num_samples: int, batch_size: int) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            for it in range(ceil(num_samples / batch_size)):
                ...
                ##########################################################
                # YOUR CODE HERE
                perturbated_inputs = inputs.repeat(this_batch_size, 1, 1, 1)
                epsilons = torch.randn_like(perturbated_inputs) * self.sigma
                perturbated_inputs = (perturbated_inputs + epsilons).clamp(0, 1)
                logits = self.base_classifier(perturbated_inputs)
                classes = torch.argmax(logits, dim=1)
                for element in classes:
                    class_counts[element] = class_counts[element] + 1
                num_remaining = num_remaining - batch_size
                ##########################################################
        return class_counts
